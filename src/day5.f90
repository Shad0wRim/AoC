...
contains
    ...
    subroutine transform_seed_list(mapping, transform)
        ...
        print *, "enter"
        ol = 1
        do while (ol < size(transform))
            innone = .true.
            do il = 1, size(mapping)
                currmap = mapping(il)
                inrng = (transform(ol) >= currmap%srcstrt)
                inrng = inrng .and. (transform(ol) <= currmap%srcstrt + currmap%rnglen - 1)

                ! in one of the maps
                if (inrng) then
                    print *, "inrng"
                    iscontained = transform(ol + 1) < currmap%rnglen - (transform(ol) - currmap%srcstrt)
                    if (iscontained) then
                    else
                        transform(ol + 1) = currmap%rnglen - (transform(ol) - currmap%srcstrt)
                        if (currmap%rnglen - transform(ol + 1) + 1 > 0) &
            ...
            if (innone) then
                print *, "innone"
                nextval = minval(mapping(:)%srcstrt, mask=mapping(:)%srcstrt > transform(ol))
                if (nextval - transform(ol) < transform(ol + 1)) then
    ...
    subroutine populate_maps(seeds, seedtosoil, soiltofert, ferttowat, wattolight, lighttotemp, temptohum, humtoloc)
        ...
        read (*, *) filename
        if (trim(filename) == 't') filename = 'test.txt'
        if (trim(filename) == 'm') filename = 'day5.txt'
        ...
        do
            ...
            case (0)
                if (scan(line, ':') /= 0) then
                    counter = 1
                    if (index(line, 'seeds:') /= 0) read (line(7:), *) seeds
                    if (index(line, 'seed-to-soil') /= 0) currarray => seedtosoil
                    if (index(line, 'soil-to-fert') /= 0) currarray => soiltofert
                    if (index(line, 'fertilizer-t') /= 0) currarray => ferttowat
                    if (index(line, 'water-to-lig') /= 0) currarray => wattolight
                    if (index(line, 'light-to-tem') /= 0) currarray => lighttotemp
                    if (index(line, 'temperature-') /= 0) currarray => temptohum
                    if (index(line, 'humidity-to-') /= 0) currarray => humtoloc
                else if (scan(line, '1234567890') /= 0) then
    ...
    subroutine calc_map_sz(filename, sizes)
        ...
        do
            ...
            select case (error)
            case (0)
                if (scan(line, ':') /= 0) ii = ii + 1
                if (index(line, 'seeds:') /= 0) then
                    do jj = 1, len_trim(line)
                        if (line(jj:jj) == ' ') sizes(1) = sizes(1) + 1
                    end do
                else if (scan(line, '1234567890') /= 0) then
                    sizes(ii) = sizes(ii) + 1
                end if
