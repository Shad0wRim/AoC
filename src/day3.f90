...
contains
    subroutine day3
        ...
        if (filename == 'm') filename = 'day3.txt'
        ...
        do i = 1, num_lines
            ...
            do while (numstart /= 0)
                is_valid = (line(numstart - 1:numstart - 1) == '*')
                is_valid = is_valid .or. (line(numstart - 1:numstart - 1) == '-')
                is_valid = is_valid .or. (line(numend + 1:numend + 1) == '*')
                is_valid = is_valid .or. (line(numend + 1:numend + 1) == '-')
                is_valid = is_valid .or. (any(schematic(numstart - 1:numend + 1, i - 1) == '*'))
                is_valid = is_valid .or. (any(schematic(numstart - 1:numend + 1, i - 1) == '-'))
                is_valid = is_valid .or. (any(schematic(numstart - 1:numend + 1, i + 1) == '*'))
                is_valid = is_valid .or. (any(schematic(numstart - 1:numend + 1, i + 1) == '-'))
        ...
        do i = 1, num_lines
            ...
            do while (scan(line, '*') /= 0)
                ...
                numadj = 0
                if (gearloc == 0) cycle
                if (scan(line(gearloc - 1:gearloc - 1), '0123456789') /= 0) then
                    numadj = numadj + 1
                    numend = gearloc - 1
                ...
                if (verify(gearline, '0123456789') == 0) then
                    numadj = numadj + 1
                    numstart = gearloc - 1
                    numend = gearloc + 1
                    read (prevline(numstart:numend), *) partnums(numadj)
                end if
                if (scan(gearline, '0123456789', back=.true.) == 1) then
                    numadj = numadj + 1
                    numend = gearloc - 1
                    numstart = verify(prevline(:gearloc - 1), '0123456789', back=.true.) + 1
                    read (prevline(numstart:numend), *) partnums(numadj)
                end if
                if (scan(gearline, '0123456789') == 1 .and. &
                    scan(gearline, '0123456789', back=.true.) == 2) then
                    numadj = numadj + 1
                    numend = gearloc
                    numstart = verify(prevline(:gearloc), '0123456789', back=.true.) + 1
                    read (prevline(numstart:numend), *) partnums(numadj)
                end if
                if (scan(gearline, '0123456789') == 2) then
                    numadj = numadj + 1
                    numstart = gearloc
                    numend = verify(prevline(numstart:), '0123456789') + numstart - 2
                    read (prevline(numstart:numend), *) partnums(numadj)
                end if
                if (scan(gearline, '0123456789') == 3) then
                    numadj = numadj + 1
                    numstart = gearloc + 1
                    numend = verify(prevline(numstart:), '0123456789') + numstart - 2
                    read (prevline(numstart:numend), *) partnums(numadj)
                ...
                if (scan(gearline, '0123456789') == 1 .and. &
                    scan(gearline, '0123456789', back=.true.) == 3 .and. &
                    scan(gearline(2:2), '0123456789') == 0) then
                    numadj = numadj + 2
                    numstart = gearloc + 1
                    numend = verify(prevline(numstart:), '0123456789') + numstart - 2
                    read (prevline(numstart:numend), *) partnums(numadj)
