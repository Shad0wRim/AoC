...
contains
   ...
   subroutine day7
      ...
      if (filename .eq. 'm') filename = 'day7.txt'
   ...
   function handtype(self)
      ...
      do i = 1, 5
         matches(i) = count(handarr .eq. handarr(i))
      end do

      ! 5 of a kind
      if (any(matches .eq. 5)) then
         handtype = 7
         return
      end if

      ! 4 of a kind
      if (any(matches .eq. 4)) then
         handtype = 6
         return
      end if

      ! full house
      if (count(matches .eq. 3) .eq. 3 .and. count(matches .eq. 2) .eq. 2) then
         handtype = 5
         return
      end if

      ! three of a kind
      if (count(matches .eq. 3) .eq. 3) then
         handtype = 4
         return
      end if

      ! two pair
      if (count(matches .eq. 2) .eq. 4) then
         handtype = 3
         return
      end if

      ! one pair
      if (count(matches .eq. 2) .eq. 2) then
         handtype = 2
         return
      end if

      ! high card
      if (all(matches .eq. 1)) then
         handtype = 1
         return
      end if
   ...
   function ishigher(self, other)
      ...
      if (handtype(self) .gt. handtype(other)) then
         ishigher = .true.
      else if (handtype(self) .lt. handtype(other)) then
         ishigher = .false.
      else
         print *, self, "  ", other
         do i = 1, 5
            ...
            if (selfrank .gt. otherrank) then
               ishigher = .true.
               return
            else if (selfrank .lt. otherrank) then
               ishigher = .false.
               return
            else
               cycle
   ...
   function handtype_j(self)
      ...
      select case (count(handarr .eq. 'J'))
      case (0)
         continue
      case (1)
   ...
   function ishigher_j(self, other)
      ...
      ishigher_j = .false.
      ! type first, then highcard from first index
      if (handtype_j(self) .gt. handtype_j(other)) then
         ishigher_j = .true.
      else if (handtype_j(self) .lt. handtype_j(other)) then
         ishigher_j = .false.
      else
         print *, self, "  ", other
         do i = 1, 5
